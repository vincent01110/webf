...
import style from './CollectionsButtons.module.css';
import { useNavigate } from 'react-router-dom';


const CollectionsButtons = (props) => {
    ...
    const accept = () => {
        toast.current.show({ severity: 'info', summary: 'Deleted', detail: 'You have deleted collection id:' + props.selected, life: 3000 });
        props.delete()
    };
    ...
    const deleteHandler = (event) => {
        if (!props.selected) {
            toast.current.show({ severity: 'info', summary: 'Invalid', detail: 'Please select a collection', life: 3000 })
        } else {
            confirmPopup({
                target: event.currentTarget,
                message: `Do you want to delete collection id:${props.selected}?`,
    ...
    const editHandler = () => {
        if (!props.selected) {
            ...
            navigate(`/admin/collection/edit/${props.selected}`)
        }
    }
